...
export default async function ProjectsWorkspace({ searchParams }: { searchParams: Promise<{ slug?: string; view?: string }> }) {
  ...
  return <div className="workspace-main"><p className="content-caption">PORTFOLIO / CASE STUDIES</p><h1>Projects</h1><p className="content-summary">作品卡片呈現成果與範圍；專案說明記錄架構與取捨。</p>
    <div className="workspace-columns project-workspace-columns"><section className="content-panel"><div className="document-toolbar"><h2>選取專案</h2><ProjectCreateForm canCreate={session.user.role === "admin"} /></div>{projects.map((p) => <a className="workspace-record" href={`/admin/projects?slug=${p.slug}`} key={p.slug}><strong>{p.title}</strong><span>{p.published === false ? "草稿" : "公開"}</span></a>)}</section>
