...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return { title: post ? `${post.problem!.id}. ${post.title} · LeetCode` : "LeetCode", description: post?.description,
...
export default async function ProblemPage({ params }: Props) {
  ...
  return <Section><Container><article className="content-panel reading-panel">
    ...
    <p className="content-caption">LEETCODE / {post.problem!.difficulty} / {post.problem!.status} / {post.problem!.language}</p>
    <h1>#{post.problem!.id} {post.title}</h1><p className="content-summary">{post.description}</p>
