...
export async function GET(request: Request, { params }: { params: Promise<{ asset: string[] }> }) {
  ...
  for (let depth = Math.min(2, asset.length - 1); depth >= 1; depth -= 1) {
    ...
    return createAssetFileResponse({ cacheControl:"public, max-age=0, must-revalidate", contentType:types[path.extname(filePath).toLowerCase()] ?? "application/octet-stream", filePath, request, stats });
  }
