...
import { getAdminResearchMediaUrl } from "@/lib/admin/research-media";
...
export async function POST(request: Request) {
  try {
    ...
    return apiSuccess({ html: await Promise.all(body.blocks.map((block) => markdownToHtml(block, body.slug ? { resolveAssetUrl:(assetPath) => getAdminResearchMediaUrl(body.slug, assetPath) } : {}))) });
