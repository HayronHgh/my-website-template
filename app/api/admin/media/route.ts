...
import { saveArticleMedia } from "@/lib/admin/media";
...
export async function POST(request: Request) {
  try {
    ...
    const length = Number(request.headers.get("content-length") ?? 0); if (!Number.isFinite(length) || length > 101 * 1024 * 1024) throw new AdminApiError(413, "request_too_large", "上傳檔案過大。");
    ...
    return apiSuccess(await saveArticleMedia(slug, file), { status: 201 });
